...
def new_transfer(param, user):
    sql = "insert into transfers (UserID, pln, pln_c, AccountNumber, FirstName, LastName, City, Address, TitleTransfer, Status) values ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')" % (user , param.get('PLN'), param.get('PLN_C'), param.get('accountnumber'), param.get('firstname'), param.get('lastname'), param.get('city'), param.get('address'), param.get('titletransfer'), "False" )
    try:
        conn = MySQLdb.connect('localhost', 'testuser', 'xxxx', 'cs_bank_2')
